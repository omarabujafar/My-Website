  ...
  return (
    <div className="flex w-[284px] items-center gap-2 py-3 pl-3.5 pr-5">
      ...
      <button
        ...
        onClick={handleReset}
        className="bg-foreground/15 hover:bg-foreground/20 flex h-10 w-10 items-center justify-center rounded-full text-white transition-colors"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
      ...
      <div className="ml-auto flex items-baseline gap-1.5 pr-0.5 text-[#F7A815]">
        ...
        <Counter paused={isPaused} resetTrigger={resetTrigger} />
      </div>
    </div>
  )
}
...
function Counter({
  ...
  return (
    <div className="relative w-[64px] overflow-hidden whitespace-nowrap text-3xl">
      0:
      <AnimatePresence initial={false} mode="popLayout">
        {countArray.map((n, i) => (
          <motion.div
            className="inline-block w-4 tabular-nums"
            key={n + i}
